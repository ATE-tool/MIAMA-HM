---
title: "Health Data"
author: "Belen Zapata-Diomedi, Marina Berdikhaniva, Steve Pemberton"
...
3.  Geographical Hierarchy (as of 2011) and table of correspondences to regions
    ...
    <https://www.ons.gov.uk/file?uri=/peoplepopulationandcommunity/populationandmigration/migrationwithintheuk/datasets/userinformationenglandandwaleslocalauthoritytoregionlookup/june2012/laregionlookup2012_tcm77-368555.xls> (downloaded 23-Oct-25) [SP NOTE: ONLY USED FOR LAD PLOT]
...
deaths_england <- deaths %>% 
  ...
  dplyr::summarize(deaths = sum(deaths),.groups = "drop") %>%
  ungroup()
...
# [Source]: https://www.ons.gov.uk/file?uri=/peoplepopulationandcommunity/populationandmigration/migrationwithintheuk/datasets/userinformationenglandandwaleslocalauthoritytoregionlookup/june2012/laregionlookup2012_tcm77-368555.xls
regions <- read_xls(here('health_data/original/ons/laregionlookup2012_tcm77-368555.xls'), 
                         sheet = 'LA_region_2012',
                    skip = 3)  ## SP NOTE: Only used for LAD plot
...
# Age-standardized Rate and relative risks by LAD
...
#  filter(population != 0) %>%
  ...
  mutate(
    ...
    RR_lad = stdrate / stdrate_ave) %>%
  ungroup()  %>%
  dplyr::select(-product, -stdrate_ave)

```

## Calculate relative risks by lsoa and sex for all cause mortality
...
## Sex specific ASR and relative risks
...
mortality_socio_gender <- england_deaths_socio %>%
  ...
  summarise(
    ...
    stdrate = mean(stdrate),  # SP NOTE - I changed this from 'asr' to 'stdrate'
    .groups = "drop"
  )  %>%
  ungroup() %>%
  arrange(gender) %>%
...
england_lifetable_lad <- deaths_lad_eng_asdr %>% 
  rename(sex = gender, population_lad = pop) %>% 
  cross_join(england_lifetable) %>% 
  filter(sex.x == sex.y) %>%
  mutate(rate1000 = rate1000*RR_lad) %>%
  select(lad_code, lad_name, sex = sex.x, age, rate1000, population_lad, RR_lad)

# Lifetable by IMD

england_lifetable_imd <- mortality_socio_gender %>% 
  rename(sex = gender, population_imd = pop) %>% 
  cross_join(england_lifetable) %>% 
  filter(sex.x == sex.y) %>%
  mutate(rate1000 = rate1000*RR_imd) %>%
  select(imd_decile, sex = sex.x, age, rate1000, population_imd, RR_imd)

# Save required files
write.csv(england_lifetable_lad, here("health_data/processed/england_lifetable_lad.csv"), 
          row.names = F)
write.csv(england_lifetable_imd, here("health_data/processed/england_lifetable_imd.csv"), 
          row.names = F)
...
### Deaths rates for England by LSOA and IMD
...
### Deaths rates for England by LAD

```{r}
# Graph death rates england by LAD

data_plot <- england_lifetable_lad %>%
  left_join(regions %>% dplyr::select(la_name, region_name), by = c("lad_name" = "la_name")) %>%
  mutate(region_name = as.factor(region_name)) 

p_england_lad <- ggplot(data_plot, 
                    aes(x = age, y = rate1000,
                        col = region_name,  # Color by region
                        group = lad_code)) +  # Each LAD has its own line
  geom_line(lwd = 0.7, alpha = 0.3) +  # Reduce opacity to manage overplotting
  facet_wrap(~ sex) +  # Compare IMD deciles within each sex
  ylab("Mortality rate (per 1000)") + 
  xlab("Age (years)") +  
  scale_y_continuous(trans = "log", breaks = c(0.2, 0.5, 1, 5, 10, 50, 100, 200)) +
  ...
  ggtitle("Death rates England by LAD")  # Add title
...
```
```{r}
# Graph death rates england by LAD, excluding London
...
p_england_ex_london_lad <- ggplot(data_plot, 
                    aes(x = age, y = rate1000,
                        col = region_name,  # Color by region
                        group = lad_code)) +  # Each LAD has its own line
  geom_line(lwd = 0.7, alpha = 0.3) +  # Reduce opacity to manage overplotting
  ...
  ggtitle("Death rates England by LAD, excluding London")  # Add title

# Print the plot
print(p_england_ex_london_lad)
...
data_plot <- england_lifetable_imd %>%
  mutate(imd_decile = as.factor(imd_decile))  # Ensure imd_decile is a factor

p_england_imd<- ggplot(data_plot, 
                    aes(x = age, y = rate1000,
                        col = imd_decile,  # Color by region
                        group = imd_decile)) +  # Each IMD decile has its own line
  geom_line(lwd = 0.7, alpha = 0.5) +  # Reduce opacity to manage overplotting
  facet_wrap(~ sex) +  # Compare IMD deciles within each sex
  ylab("Mortality rate (per 1000)") + 
  xlab("Age (years)") +  
...
### Comparison with European standard population by LADs and sex

Comparison of our calculations of age standardised rates with calculations using weight from the European standard population. We use European standard population to compare because ASDR per LAD from ONS use this weights in their calculations.

In this comparison, stdr_rate_eng is calculated by us using
England population weights and stdr_rate_eu is our estimates using
European standard population weights. 

*SP NOTE: I have omited the comparison with 2025 ONS age standardised rates (which was included in JIBE, but only for the Manchester area).*
...
# Add MSOA and LAD, and use best rate for small death numbers
...
# compare rates by LAD
deaths_lad_compare <- deaths_lad_eng_asdr %>%
  dplyr::select(lad_code, lad_name, gender, stdr_rate_eng = stdrate) %>%
  left_join(deaths_lad_eng_eu %>% rename(stdr_rate_eu = stdrate),
            by = c("lad_code", "lad_name", "gender")) %>%
  mutate(abs_diff = abs(stdr_rate_eng - stdr_rate_eu))

print(deaths_lad_compare)
  
# graph of differences
p_deaths_lad_compare <- ggplot(deaths_lad_compare, 
                               aes(x = abs_diff)) +
  geom_histogram(bins = 30, fill = "steelblue", 
                 colour = "white", alpha = 0.8) +
  facet_wrap(~ gender) +
  labs(
    title = "Comparison with European standard population by LAD",
    subtitle = "Distribution of Absolute Differences in age-standardised rates",
    x = "Absolute Difference (abs_diff)",
    y = "Count"
  ) +
  theme_minimal(base_size = 14)

print(p_deaths_lad_compare)

```

# Disease Incidence - BELOW HERE NOT ADDRESSED YET
