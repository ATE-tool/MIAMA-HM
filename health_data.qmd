...
england_lsoa21_syoa_cal <- england_lsoa21_syoa_cal %>% 
  left_join(deprivation, by= c("lsoa21cd" = "lsoa_code")) %>% 
  select(lsoa21cd, age, sex= gender, rate = rate_cal, imd_decile, deprivation, LAD22NM) %>%
  mutate(rate1000= rate*1000)

readr::write_csv(
  england_lsoa21_syoa_cal,
  here::here("health_data/processed/england_lsoa21_mxrates_calibrated.csv")
)

```

## Lifetables by LAD and IMD

```{r}
## SP note: can't aggregate using mean weighted by population  for ages over 100,
## as we only have populations up to 90

## Issues to resolve:
## - handling ages above 90 (could just make it mean instead of weighted mean?)
## - areas with NA for their deprivation

# lifetable by LAD and IMD, based on calibrated results
england_lifetable_lad_imd <- england_lsoa21_syoa_cal %>%
  
  # join population
  left_join(pop21_syoa, by = c("lsoa21cd",  "sex" = "gender", "age")) %>%
  
  # # remove NAs (which are ages > 90)
  # filter(!is.na(population)) %>%
  
  # alternative, so that ages over 90 are retained but effectively unweighted,
  # by giving all LSOAs the same weight (nominal value of 1)
  mutate(population = ifelse(age > 90 & is.na(population),
                             1, population)) %>%
  
  # aggregate to LAD and IMD
  group_by(LAD22CD, LAD22NM, imd_decile, age, sex) %>%
  summarise(rate1000 = weighted.mean(rate1000, w = population),
            rate = weighted.mean(rate, w = population),
            population = sum(population),
            .groups = "drop")

# save output
write.csv(
  england_lifetable_lad_imd, 
  here("health_data/processed/england_lifetable_lad_imd.csv"),
  row.names = FALSE
)
...
```{r}
# Graph death rates england by LAD and IMD

data_plot_lad_imd <- england_lifetable_lad_imd %>%
  mutate(lad_imd = paste(LAD22NM, imd_decile, sep = "_")) %>%
  mutate(imd_decile = as.factor(imd_decile))  # Ensure imd_decile is a factor

p_lad_imd<- ggplot(data_plot_lad_imd, 
                   aes(x = age, y = rate1000,
                       col = imd_decile,  # Color by imd_decile
                       group = lad_imd)) +  # Each LAD/IMD combination has its own line
  geom_line(lwd = 0.7, alpha = 0.5) +  # Reduce opacity to manage overplotting
  facet_wrap(~ sex) +  # Compare IMD deciles within each sex
  ylab("Mortality rate (per 1000)") + 
  xlab("Age (years)") +  
  ...
  ggtitle("Death rates England by LAD and IMD decile")  # Add title
...
### Comparison with European standard population by LADs and sex - TO BE REDONE
