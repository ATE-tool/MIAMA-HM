...
2.  Population Data by LSOA by age and sex for 2018 mid year, and 2023 mid year [NOT CURRENTLY USED] (downloaded 12-Nov-25), from ONS.
...
# SP NOTE: not currently used, but option to use it exists in disease data
...
We summarise death rates in two ways: (1) grouping by IMD decile (weighted mean of the rates for all LSOAs in each IMD decile), and (2) finding the middle rate of the deciles for all LSOAs combined.  In each case, these are based on the calibrated and smoothed rates as calculated above.

```{r deathrates-grouped}
...
# death rates by IMD decile
england_deathrates_imd <- england_lsoa21_syoa_smoothed %>%
...
# save output
saveRDS(england_deathrates_imd, here("health_data/processed/england_imd_mxrates.rds"))

# deathrates by decile midrates
england_deathrates_decilemid <- england_lsoa21_syoa_smoothed %>%
  dplyr::select(lsoa21cd, sex, age, rate1000, imd_decile) %>%
  ...
  # aggregate to mid-rate of each decile
  group_by(age, sex) %>%
    summarise(
    percentile_5 = wtd.quantile(rate1000, weights = population, probs = 0.05),
    percentile_15 = wtd.quantile(rate1000, weights = population, probs = 0.15),
    percentile_25 = wtd.quantile(rate1000, weights = population, probs = 0.25),
    percentile_35 = wtd.quantile(rate1000, weights = population, probs = 0.35),
    percentile_45 = wtd.quantile(rate1000, weights = population, probs = 0.45),
    percentile_55 = wtd.quantile(rate1000, weights = population, probs = 0.55),
    percentile_65 = wtd.quantile(rate1000, weights = population, probs = 0.65),
    percentile_75 = wtd.quantile(rate1000, weights = population, probs = 0.75),
    percentile_85 = wtd.quantile(rate1000, weights = population, probs = 0.85),
    percentile_95 = wtd.quantile(rate1000, weights = population, probs = 0.95),
    # population = sum(population),
    .groups = "drop") %>%
  pivot_longer(
  ...
  dplyr::select(percentile, age, sex, rate1000)


# save output
saveRDS(england_deathrates_decilemid, here("health_data/processed/england_decilemid_mxrates.rds"))
...
# Graph death rates for England by LSOA

data_plot <- england_lsoa21_syoa_smoothed %>%
  mutate(colour_col = as.factor(imd_decile),
         group_col = lsoa21cd) 

p <- rateplot(data_plot, 
              ylab_plot = "Mortality rate (per 1000)",
              legend_name = "IMD Decile",
              plot_title = "Death rates England")

print(p)

```

### Death rates by IMD deciles

```{r graph-death-imd}

# Graph death rates for England by IMD decile

data_plot <- england_deathrates_imd %>%
  mutate(colour_col = as.factor(imd_decile),
         group_col = as.factor(imd_decile))

p <- rateplot(data_plot, 
              ylab_plot = "Mortality rate (per 1000)",
              legend_name = "IMD Decile",
              plot_title = "Death rates England by IMD decile")

print(p)

```

### Death rates by decile midrates

```{r graph-death-decmid}

# Graph death rates for England by decile midrates

data_plot <- england_deathrates_decilemid %>%
  mutate(colour_col = as.factor(as.numeric(percentile)),
         group_col = as.factor(as.numeric(percentile)))

p <- rateplot(data_plot, 
              ylab_plot = "Mortality rate (per 1000)",
              legend_name = "Percentile",
              plot_title = "Death rates England by decile midrates")
...
output_dir <- here("health_data/images/inc_gbd_age_sex")
...
## Apply relative risks, add deprivation data
...
# Output tables are very large, so lean up before creating
rm(list = setdiff(ls(), 
                  c(lsf.str(), ## keeps functions
                    "england_lsoa_deaths", 
                    "deprivation_lsoa11",  # update to 2021 if mortality is updated
                    "population_england_syoa", # replace with 2023 pop if changed below
                    "gbdpyr_interp")))  

# We calculate incidence and prevalence separately, using the same functions, 
# to avoid memory overflow issues as the tables are very large

# Function to apply LSOA relative risks (as calculated for deaths) to disease data
# Note that this is 2011 LSOAs (based on the 'risks' table which is 
# england_lsoa_deaths) - this should not matter as the results are aggregated;
# the issue would disappear if deaths were re-done to use 2023 values
# on 2021 LSOA boundaries
calculateRRs <- function(interp_data, risks) {
  output <- interp_data %>%
    ...
    left_join(deprivation_lsoa11 %>% dplyr::select(lsoa11_code, imd_decile), 
              by = c("lsoa_code" = "lsoa11_code"))
...
# calculate total pop
# currently using 2018 population (for 2011 LSOAs)
age_90_data <- population_england_syoa %>% filter(age == 90)
...
# combine with population to age 90
pop_stretched <- bind_rows(population_england_syoa, new_rows) %>%
  rename(sex = gender) %>%
  dplyr::select(-lsoa_name)

# # but better, if england_lsoa_deaths were re-done based on 2023 mortality data and
# # 2021 LSOAs, would be to use 2023 pop data (for 2021 LSOAs)
...
# function to group by IMD deciles
groupImd <- function(diseasetable, pop) {
  output <- diseasetable %>%
    
    # join population
    left_join(pop, by = c("lsoa_code", "sex", "age")) %>%
    
    # remove where zero population
    filter(population > 0) %>%
    
    # aggregate to IMD
    group_by(cause, imd_decile, age, sex) %>%
    summarise(rate1000 = weighted.mean(rate1000, w = population),
              .groups = "drop")
...
# function to group by decile midrates
groupDecMid <- function(diseasetable, pop) {
  output <- diseasetable %>%
  ...
  mutate(percentile = gsub("percentile_", "", percentile)) %>%
  dplyr::select(cause,percentile, age, sex, rate1000)
...
#### Incidence rates by LSOA

```{r inc-lsoa}

# apply relative risks to disease incidence data
england_incidencetable_lsoa <- calculateRRs(gbdpyr_interp %>%
                                              filter(measure == "Incidence"),
                                            england_lsoa_deaths)

# save output
saveRDS(england_incidencetable_lsoa, here("health_data/processed/england_lsoa11_incidencerates.rds"))
  
```

#### Incidence rates grouped

We summarise incidence rates in two ways: (1) grouping by IMD decile (weighted mean of the rates for all LSOAs in each IMD decile), and (2) finding the middle rate of the deciles for all LSOAs combined. 

```{r inc-imd}

# disease incidence table by IMD deciles
england_incidencetable_imd <- groupImd(england_incidencetable_lsoa, pop_stretched)
  
# save output
saveRDS(england_incidencetable_imd, 
        here("health_data/processed/england_imd_incidencerates.rds"))

# disease incidence table by decile midrates (very slow - couple of hours)
england_incidencetable_decilemid <- groupDecMid(england_incidencetable_lsoa, pop_stretched)
  
# save output
saveRDS(england_incidencetable_decilemid, 
        here("health_data/processed/england_decilemid_incidencerates.rds"))
...
Plots of disease incidence rates by LSOA are saved in the `images/inc_lsoa` folder.  
...
# Define output directory
output_dir <- here("health_data/images/inc_lsoa")
if (!dir.exists(output_dir)) {
  dir.create(output_dir, recursive = TRUE)
}

for (i in unique(england_incidencetable_lsoa$cause)) {
  
  data_plot <- england_incidencetable_lsoa %>%
    filter(cause == i) %>%
    mutate(colour_col = as.factor(imd_decile),
           group_col = lsoa_code) 
  
  p <- rateplot(data_plot, 
                ylab_plot = "Disease incidence rate (per 1000)",
                legend_name = "IMD Decile",
                plot_title = paste("Incidence rates England:", i))

  ggsave(filename = paste0(output_dir, "/plot_", i, ".png"), plot = p, width = 12, height = 6)

}

```

##### Incidence rates by IMD deciles

Plots of disease incidence rates by IMD deciles are saved in the `images/inc_imd_decile` folder.  
...
# Define output directory
output_dir <- here("health_data/images/inc_imd_decile")
if (!dir.exists(output_dir)) {
  dir.create(output_dir, recursive = TRUE)
}

for (i in unique(england_incidencetable_imd$cause)) {

  data_plot <- england_incidencetable_imd %>%
    filter(cause == i) %>%
    mutate(colour_col = as.factor(imd_decile),
           group_col = as.factor(imd_decile)) 
  ...
  p <- rateplot(data_plot, 
                ylab_plot = "Disease incidence rate (per 1000)",
                legend_name = "IMD Decile",
                plot_title = paste("Incidence rates England by IMD decile:", i))


  ggsave(filename = paste0(output_dir, "/plot_", i, "_imd.png"), plot = p, width = 12, height = 6)

}

```
##### Incidence rates by decile midrates

Plots of disease incidence rates by decile midrates are saved in the `images/inc_decile_midrate` folder.  

```{r graph-incidence-dec-midrate}

# graphs of incidence of disease rates, by decile midrates

# Define output directory
output_dir <- here("health_data/images/inc_decile_midrate")
if (!dir.exists(output_dir)) {
  dir.create(output_dir, recursive = TRUE)
}

for (i in unique(england_incidencetable_decilemid$cause)) {

  data_plot <- england_incidencetable_decilemid %>%
    filter(cause == i) %>%
  mutate(colour_col = as.factor(as.numeric(percentile)),
         group_col = as.factor(as.numeric(percentile)))
  ...
  p <- rateplot(data_plot, 
                ylab_plot = "Disease incidence rate (per 1000)",
                legend_name = "Percentile",
                plot_title = paste("Incidence rates England by decile midrates:", i))

  ggsave(filename = paste0(output_dir, "/plot_", i, "_decmid.png"), plot = p, width = 12, height = 6)
...
# remove the large incidence tables
rm(england_incidencetable_lsoa, england_incidencetable_imd, 
   england_incidencetable_decilemid)

```

### Prevalence

#### Prevalence rates by LSOA

```{r prev-lsoa}

# apply relative risks to disease prevalence data
england_prevalencetable_lsoa <- calculateRRs(gbdpyr_interp %>%
                                               filter(measure == "Prevalence"),
                                             england_lsoa_deaths)

# save output
saveRDS(england_prevalencetable_lsoa, here("health_data/processed/england_lsoa11_prevalencerates.rds"))
  
```

#### Prevalence rates grouped

We summarise prevalence rates in two ways: (1) grouping by IMD decile (weighted mean of the rates for all LSOAs in each IMD decile), and (2) finding the middle rate of the deciles for all LSOAs combined. 

```{r prev-imd}

# disease prevalence table by IMD deciles
england_prevalencetable_imd <- groupImd(england_prevalencetable_lsoa, pop_stretched)
  
# save output
saveRDS(england_prevalencetable_imd, 
        here("health_data/processed/england_imd_prevalencerates.rds"))

# disease prevalence table by decile midrates (very slow - couple of hours)
england_prevalencetable_decilemid <- groupDecMid(england_prevalencetable_lsoa, pop_stretched)
  
# save output
saveRDS(england_prevalencetable_decilemid, 
        here("health_data/processed/england_decilemid_prevalencerates.rds"))
...
Plots of disease prevalence rates by LSOA are saved in the `images/prev_lsoa` folder.  
...
# Define output directory
output_dir <- here("health_data/images/prev_lsoa")
if (!dir.exists(output_dir)) {
  dir.create(output_dir, recursive = TRUE)
}

for (i in unique(england_prevalencetable_lsoa$cause)) {
  
  data_plot <- england_prevalencetable_lsoa %>%
    filter(cause == i) %>%
    mutate(colour_col = as.factor(imd_decile),
           group_col = lsoa_code) 
  
  p <- rateplot(data_plot, 
                ylab_plot = "Disease prevalence rate (per 1000)",
                legend_name = "IMD Decile",
                plot_title = paste("prevalence rates England:", i))
...
##### Prevalence rates by IMD deciles

Plots of disease prevalence rates by IMD deciles are saved in the `images/prev_imd_decile` folder.  
...
# Define output directory
output_dir <- here("health_data/images/prev_imd_decile")
if (!dir.exists(output_dir)) {
  dir.create(output_dir, recursive = TRUE)
}

for (i in unique(england_prevalencetable_imd$cause)) {

  data_plot <- england_prevalencetable_imd %>%
    filter(cause == i) %>%
    mutate(colour_col = as.factor(imd_decile),
           group_col = as.factor(imd_decile)) 
  ...
  p <- rateplot(data_plot, 
                ylab_plot = "Disease prevalence rate (per 1000)",
                legend_name = "IMD Decile",
                plot_title = paste("prevalence rates England by IMD decile:", i))
...
##### Prevalence rates by decile midrates

Plots of disease prevalence rates by decile midrates are saved in the `images/prev_decile_midrate` folder.  

```{r graph-prevalence-dec-midrate}

# graphs of prevalence of disease rates, by decile midrates

# Define output directory
output_dir <- here("health_data/images/prev_decile_midrate")
if (!dir.exists(output_dir)) {
  dir.create(output_dir, recursive = TRUE)
}

for (i in unique(england_prevalencetable_decilemid$cause)) {

  data_plot <- england_prevalencetable_decilemid %>%
    filter(cause == i) %>%
  mutate(colour_col = as.factor(as.numeric(percentile)),
         group_col = as.factor(as.numeric(percentile)))
  ...
  p <- rateplot(data_plot, 
                ylab_plot = "Disease prevalence rate (per 1000)",
                legend_name = "Percentile",
                plot_title = paste("prevalence rates England by decile midrates:", i))
...
```{r cleanup-prevalence}

# remove the large prevalence tables
rm(england_prevalencetable_lsoa, england_prevalencetable_imd, 
   england_prevalencetable_decilemid)
...
Saves the files required by the health model: `health_transitions_prevalence.csv` and `health_transitions_raw.csv`. 

Note that the output rates are raw rates.  *We may later adjust them for levels of physical activity in the population.*

```{r combination}

allcause <- readRDS(here("health_data/processed/england_imd_mxrates.rds"))
incidence <- readRDS(here("health_data/processed/england_imd_incidencerates.rds"))
prevalence <- readRDS(here("health_data/processed/england_imd_prevalencerates.rds"))
...
write.csv(health_transitions, here("health_data/processed/health_transitions.csv"),
          row.names = FALSE) 
